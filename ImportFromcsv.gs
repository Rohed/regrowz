...
function saveOrder2(data, row, recipes) {
    ...
    var RECIPE = recipes[data.recipe.id];
...
saveFileCsv2(null, null, 14002, 2000) 

}
function saveFileCsv2(data, name, sheetRow, numRows) {
    var LOGDATA = {
        status: true,
        msg: '',
        action: 'Import CSV',
...
//     return '';
//     var sheet = SpreadsheetApp.openById('1X71XaKrhg2ZCMusgujmm59azPZCz7k3SWWx12Slkhao').getSheets()[0];
               ...
               // var dataPC = base.getData("References/ProductCodes/" + PC);
                ...
                if (dataPC) {
                    ...
                    var item = {
                        ...
                        fill: dataPC.fill,
                        orderdate: values[i][8] ? new Date( values[i][8]) ? new Date( values[i][8]).getTime() : ''  : '',
                        //    batch: values[i][0],
                        priority: '',
                        customer: values[i][2],
                ...
                if (item.recipe.name) {
                    ...
                    var resp = saveOrder2(item, row,recipes);
...
//     SpreadsheetApp.openById('10lo4vLIJONltnwO_n1Va5ixkIE7_Z4Y-7DEZRE5_8xc').getSheets()[0].getRange(1, 1, values.length, values[0].length).clear().setValues(values);
//     return '';
     var values =SpreadsheetApp.openById('1X71XaKrhg2ZCMusgujmm59azPZCz7k3SWWx12Slkhao').getSheets()[0].getDataRange().getValues();
